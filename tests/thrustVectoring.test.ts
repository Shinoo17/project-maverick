...
import { createThrustVectoringState, f22ThrustForces, f22TvcProfile, f22TvcTargets, stepThrustVectoring } from '../src/game/flight/thrustVectoring'
...
function actuate(s: ReturnType<typeof state>, input: ReturnType<typeof command>, seconds: number, _speed = 130, _aoa = 0) {
  for (let i = 0; i < seconds / FLIGHT_STEP; i++) stepThrustVectoring(s, f22TvcTargets(input, 1), FLIGHT_STEP)
}
...
describe('F-22 continuous simulation TVC', () => {
  ...
  it('slews with inertia to near both limits and returns smoothly on release at any limiter opening', () => {
    for (const limiterOpen of [0, 0.5, 1]) {
      ...
      for (let i = 0; i < 360; i++) {
        stepThrustVectoring(s, f22TvcTargets(command(-1, -1), 1), FLIGHT_STEP)
      ...
      stepThrustVectoring(s, f22TvcTargets(command(), 1), FLIGHT_STEP)
  ...
  it('integrates actual actuator torque into attitude even after release and preserves zero-thrust behavior', () => {
    ...
    expect(s.rates.pitch).toBeCloseTo(force.angularAcceleration.pitch * f22TvcProfile.gain * FLIGHT_STEP, 10)
    ...
    off.velocity = { x: 0, y: 0, z: 0 }; off.enginePower = 0; off.engine.actualThrust = 0
  ...
  it('freezes, single-steps and resets the actuators with the simulation clock', () => {
    ...
    r.singleStep(powered)
    expect(r.snapshot().aircraft[0].thrustVectoring).not.toEqual(paused.aircraft[0].thrustVectoring)
