...
import { aircraftIds, scenarioNames, replayGolden, compareNumericLeaves, createAircraft, type Golden } from '../../benchmarks/flight/harness'
...
describe.each(aircraftIds)('%s Phase 0 golden tracks', aircraftId => {
  it.each(scenarioNames)('%s reproduces recorded numeric leaves and is exact at 30/60/144 FPS', scenario => {
    ...
    for (const sample of golden.samples) {
      expect(compareNumericLeaves(replay.samples[sample.step].state, sample.state), `${scenario}/${aircraftId} step ${sample.step}`).toEqual([])
    }
...
it('compares numeric leaves with relative tolerance, ignoring new fields only', () => {
  expect(compareNumericLeaves({ x: 100 + 1e-8, added: 20 }, { x: 100 })).toEqual([])
  expect(compareNumericLeaves({ x: 100 + 1e-6 }, { x: 100 })).not.toEqual([])
  expect(compareNumericLeaves({}, { x: 0 })).not.toEqual([])
  expect(compareNumericLeaves({ x: NaN }, { x: 0 })).not.toEqual([])
})
