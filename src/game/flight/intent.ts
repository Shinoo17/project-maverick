...
/** Player inputs only. Saturation is controller feedback against the closed limiter,
...
export function readIntent(command: PilotCommand, previous: PilotIntent, dt: number, requestedToLimitedRate: number): PilotIntent {
  ...
  return {
    demand, saturation,
    sustained: previous.sustained + (demand * saturation - previous.sustained) * (1 - Math.exp(-dt / 0.4)),
