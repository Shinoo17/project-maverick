...
interface Props { aircraftId: AircraftId; session: FlightSession; onReady: () => void; onTelemetry: (state: AircraftState) => void }
function FlightWorld({ aircraftId, session, onReady, onTelemetry }: Props) {
  ...
  useFrame(({ camera }, dt) => {
    ...
    if (session.running) {
      ...
      alpha = runtime.advance(dt, (tick, id) => {
        previous.current = runtime.snapshot().aircraft[0]
