...
export function runPsmNeutralRelease(aircraftId: string, speedAdjust: number, seconds = 40, observer?: StepObserver) {
  ...
  const held = runTrack(state, 5, () => ({ psmArm: true, pitch: 1, speedAdjust: 1 }), observer, 'heldC5')
  const trace = runTrack(held.samples.at(-1)!.state, seconds, () => ({ speedAdjust }), observer, 'neutralAfterC5')
  return { held, trace }
}
